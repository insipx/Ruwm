...
impl fmt::Display for ConfigError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ConfigError::CouldNotOpenConfigFile(ref err) => {
                write!(f, "Could Not Open Config File {}", err)
            }
            ConfigError::CouldNotParseConfigFile(ref err) => {
                write!(f, "Could Not Parse Config File {}", err)
            }
            ConfigError::FoundDuplicateVariable(ref err) => {
                write!(f, "Found Duplicate Variable: '{}'", err)
            }
            ConfigError::VariableNotFound(ref err) => write!(f, "Variable '{}' Not Found ", err),
            ConfigError::MultipleSymbols(ref err) => {
                write!(f, "More Than One Symbol Attached to Variable '{}'", err)
            }      
        }
    }
}
