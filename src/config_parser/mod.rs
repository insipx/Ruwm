...
impl Variables {
    ...
    pub fn set(&mut self, v: String, s: Vec<Value>) -> Result<(), ConfigError> {
        match self.0.contains_key(&v) {
            true => Err(ConfigError::FoundDuplicateVariable(
                DuplicateVariableError { v: v.to_string() },
            )),
            false => {
    ...
    pub fn get(&self, k: &String) -> Result<Vec<&str>, ConfigError> {
        match self.0.get(k) {
            ...
            None => Err(ConfigError::VariableNotFound(
                VariableNotFoundError { v: k.to_string() },
            )),
        }
    }
    ...
    pub fn get_single(&self, k: &String) -> Result<&str, ConfigError> {
        match self.0.get(k) {
            Some(s) => {
                if s.len() > 1 {
                    return Err(ConfigError::MultipleSymbols(
                        MultipleSymbolsError { v: k.clone() },
                    ));
                }
                match s[0] {
